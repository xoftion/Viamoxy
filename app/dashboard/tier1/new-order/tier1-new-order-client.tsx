...
import { addTier1OrderAction } from "@/app/actions/tier1"
...
interface Tier1NewOrderClientProps {
  services: Service[]
  categories: string[]
  balance: number
}

export function Tier1NewOrderClient({ services, categories, balance }: Tier1NewOrderClientProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const result = await addTier1OrderAction(
        selectedService.service,
        selectedService.name,
        link,
        Number.parseInt(quantity),
        Number.parseFloat(selectedService.rate),
      )
      ...
      toast.success(
        `Order placed! Total: ₦${breakdown.total.toLocaleString()} (Tax: ₦${breakdown.tax.toLocaleString()})`,
      )
      setTimeout(() => {
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    {selectedCategory ? "Choose a service from the list" : "Select a category to continue"}
                  </CardDescription>
                </div>
                {selectedCategory && (
                  <Button
